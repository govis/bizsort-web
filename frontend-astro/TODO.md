...
- [x] **`viewmodel/search/home.ts` — MVVM violation**: `firstUpdated()` uses `shadowRoot.querySelector('search-category-input')` to reach a child viewmodel via DOM. Fixed: children now fire a `model-ready` CustomEvent on `firstUpdated()`; parent listens and calls `attachInputs()` reactively. Removed redundant DOM writes in `_onTabSelect` and geo-toggle handler.
