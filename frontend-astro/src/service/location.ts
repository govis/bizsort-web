...
export async function autocomplete(parent: number, name: string, scope?: IdName): Promise<Autocomplete[]> {
    ...
    const response = await apiFetch(`/api/location/autocomplete?parent=${parent}&name=${encodeURIComponent(name)}${scopeParam}`);
...
export async function get(location: number, type?: number): Promise<any> {
    ...
    const response = await apiFetch(`/api/location/${endpoint}?location=${location}${typeParam}`);
...
export async function populateChildren(parent: number, type: SubType, memberType: number): Promise<Node> {
    const response = await apiFetch(`/api/location/populate_Children?parent=${parent}&type=${type}&memberType=${memberType}`);
...
export async function getName(locationId: number): Promise<IdName> {
    const response = await apiFetch(`/api/location/getName?locationId=${locationId}`);
...
export async function populatePath(location: number, street?: number): Promise<any> {
    ...
    const response = await apiFetch(`/api/location/${endpoint}`);
