...
import '../components/offering/featured';

/**
 * Offering Home page.
 * Ported from legacy offering/home.ts.
 */
export class OfferingHome extends LitElement {
  ...
  private _searchOptions = [
    { value: 1, text: "Offerings", selected: true },
  ...
  private _handleSearch(e: CustomEvent) {
    ...
    if (category || (query && query.trim() !== '')) {
      Offering.search(tType, category, query, location, near);
      return;
    }

    const featured = this.shadowRoot?.querySelector('offering-featured') as any;
    if (featured) {
      featured.selection = {
        category: category ?? 0,
  ...
  static styles = css`
    :host {
      ...
      min-height: 100vh;
      /* Background image — matching legacy offering-home-bg.jpg */
      background:
  ...
  render() {
    return html`
      ...
      <div class="featured-area">
        <offering-featured></offering-featured>
      </div>
...
if (!customElements.get('offering-home')) {
  customElements.define('offering-home', OfferingHome);
}
