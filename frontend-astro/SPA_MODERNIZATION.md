...
| Programmatic Nav (`Shell.go()`) | **Astro `navigate()` API** | Legacy helpers like `Company.search()` are preserved in `src/navigation.ts`, but now they simply pipe into Astro's `navigate(href)` to trigger a soft View Transition instead of manually manipulating `history.pushState`. |
