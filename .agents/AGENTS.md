...
### 8. Preserving Pristine Operations & Signatures (CRITICAL)
- **Do Not Break Legacy Signatures:** When modifying services, components, or backend models, NEVER assume a complex signature can be simplified. For example, if a legacy method passed discrete primitive parameters (e.g., index, length, category, location) and packed them into a specific JSON object (like DirectorySliceInput), do not rewrite the method to accept a raw sliceInput: any. This destroys type mapping and will cause catastrophic JsonException deserialization failures on the .NET backend.
- **Precision Overwrites (No Mass Replacements):** When asked to remove specific dead code (e.g., stripping out a disabled cache), use targeted deletions or targeted replace tools. **NEVER** replace or wipe out the entire file contents just to remove one class. You risk silently deleting adjacent operational ported code (like getFeatured, 	oPreview, getAffiliations).
- **No Visual Debug Artifacts:** Do not commit hardcoded debugging UI artifacts (like <div style="background: black;">Rendered with Astro</div>) into the codebase. You may use console.log for debugging logic, but visual banners must be stripped out before resolving a task.
- **Verify Missing Code Against Trackers:** If a function, service, or component suddenly appears to be missing, **double and triple-check the trackers (e.g., LEGACY_FRONTEND_TRACKER.md) and Git history** before scaffolding it from scratch. It is highly likely it was already ported and was accidentally deleted, moved, or renamed. Never assume something needs to be written from scratch without verifying its official porting status first.
