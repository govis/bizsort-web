...
- **Global Theme CSS (CRITICAL):** WebAwesome requires its base theme CSS to be imported globally to render properly. Without it, none of the internal `color-mix()` functions, `--wa-color-neutral-*` variables, or shadow DOM base styles will load, causing components to appear completely unstyled. In Astro, ensure `import '@awesome.me/webawesome/dist/styles/themes/default.css';` is present in the `Layout.astro` frontmatter.
...
### 10. CSS-Driven Animations (Replacing JS Mixins)
- **Legacy Approach:** The legacy architecture heavily relied on JavaScript mixins (e.g., `EntryAnimation`, `HeroRippleAnimatable`) and manual DOM events (`@animationend`) to sequence entry animations. For instance, `CompanyListView` applied staggered inline `animation-delay` styles via JS mapping over items.
- **Modernized Approach:** We have fully eliminated these JS animation mixins in favor of native CSS animations and pseudo-selectors. 
- **Implementation (Staggering):** When rendering lists of cards (like `<company-featured>`), use CSS `@keyframes` (e.g., `card-enter`) combined with `:nth-child(4n + x)` selectors in your Lit `static styles` to stagger the delays natively without any JavaScript overhead. Using the `4n + x` formula ensures that when infinite-scrolling or "Load More" appends new pages of 4 items, the new batch perfectly staggers without accumulating massive delays.
- **Implementation (Modals/Overlays):** For complex pop-ins (formerly "Hero" or "Ripple" FLIP animations), we now rely on simple CSS `transform: scale()` or `transform: translateY()` spring transitions driven by class toggles (like `.open`) instead of complex coordinate-mapping JS logic.
- **Implementation (Heavy Content Jank / Iframe lazy-loading):** To avoid skipping animation frames when inserting heavy DOM elements (like `<iframe />`), always decouple the heavy load from the animation thread. When using `<wa-dialog>`, defer rendering the heavy payload until the `wa-after-show` event fires (using a state toggle like `_renderIframe`).
