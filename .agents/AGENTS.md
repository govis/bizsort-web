...
- **Harmless Legacy Code is Still Pristine:** If legacy code contains technically redundant but harmless patterns (e.g., checking if (!response.ok) when the piFetch wrapper already throws), you may leave it alone if asked to preserve pristine code. Do not break working code purely to satisfy a linting ideology unless explicitly tasked to do so.
