$indexer = Get-Content "C:\Bizsort\bizsort-web\background\Workers\Company\Indexer.cs" -Raw
$indexer = $indexer -replace '(?s)\}\.\.\."\);\s+await Task\.Delay.*?\}', "}"
Set-Content "C:\Bizsort\bizsort-web\background\Workers\Company\Indexer.cs" $indexer
...
using Dictionary = BizSrt.Data.Master.Dictionary;

namespace BizSrt.Api.Process
{
    public static class Company
    {
        public enum FacetValueType : byte
        {
            _Category = 0,
            _Type = 1,
            _Industry = 2,
        ...
        public static async Task IndexCompanyAsync(AppDbContext dc, int companyId, CancellationToken cancellationToken = default)
        {
            ...
            var cache = await dc.CompanyFacets
                .Where(bf => bf.Company == companyId) 
                .ToArrayAsync(cancellationToken);

            var facets = new List<LocalFacet>();
        ...
        private static void createCompanyFacet(List<LocalFacet> facets, CompanyProfile company)
        {
            if (company.Category > 0)
            {
                facets.Add(new LocalFacet
                {
                    ValueType = FacetValueType._Category,
                    Name = "Category",
            ...
            if (company.TransactionType != 0)
            {
                ...
                foreach (var transactionType in transactionTypes)
                {
                    if ((transactionType.ItemKey & company.TransactionType) > 0)
                    {
                        facets.Add(new LocalFacet
                        {
                            ValueType = FacetValueType._Type,
                            Name = "Type",
            ...
            if (company.Industry != 0)
            {
                ...
                foreach (var industry in industries)
                {
                    if ((industry.ItemKey & company.Industry) > 0)
                    {
                        facets.Add(new LocalFacet
                        {
                            ValueType = FacetValueType._Industry,
                            Name = "Industry",
...
"@
Set-Content "C:\Bizsort\bizsort-web\backend\Process\Company.cs" $company
