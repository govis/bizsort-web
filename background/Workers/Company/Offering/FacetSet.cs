...
namespace BizSrt.Worker.Company.Offering
{
    ...
    /// <summary>
    /// Background worker that polls for un-indexed or stale offering facet sets.
    /// Ported from legacy Engine.Offering.FacetSet
    /// </summary>
    public class FacetSet : AsyncQueueWorker<FacetSetWorkItem>
    {
        ...
        protected override async Task<FacetSetWorkItem[]> RecallAsync(int maxRecords, CancellationToken cancellationToken)
        {
            ...
            var queryIndex = from pfs in dbContext.CompanyOfferingFacetSets
            ...
            if (items.Count < maxRecords)
            {
                ...
                var queryDelete = from pfs in dbContext.CompanyOfferingFacetSets
            ...
            try
            {
                switch (workItem.Action)
                {
                    case FacetSetWorkItem.ActionType.Index:
                        _logger.LogInformation($"Sending gRPC request to index Offering FacetSet {workItem.SetId}...");
                        await _grpcClient.IndexOfferingFacetSetAsync(new BizSrt.Model.Grpc.IndexOfferingFacetSetRequest { SetId = workItem.SetId }, cancellationToken: cancellationToken);
                        _logger.LogInformation($"Successfully indexed Offering FacetSet {workItem.SetId}.");
                        break;
                    case FacetSetWorkItem.ActionType.Delete:
                        _logger.LogInformation($"Sending gRPC request to delete Offering FacetSet {workItem.SetId}...");
                        await _grpcClient.DeleteOfferingFacetSetAsync(new BizSrt.Model.Grpc.DeleteOfferingFacetSetRequest { SetId = workItem.SetId }, cancellationToken: cancellationToken);
                        _logger.LogInformation($"Successfully deleted Offering FacetSet {workItem.SetId}.");
                        break;
                }
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, $"Failed to process Offering FacetSet {workItem.SetId} via gRPC.");
                throw;
            }
        }
    }
