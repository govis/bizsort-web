...
{
    public class FeaturedOfferingsCache : FeaturedCache<long[]>
    {
        public FeaturedOfferingsCache() { }

        protected override long[] FetchItems(Tuple<short, int> key)
        {
            using var dbContext = BizSrt.Api.Data.Cache.LegacyCache.GetDbContext();

            var pq = dbContext.Offerings.Where(p => p.Status == (byte)BizSrt.Model.Offering.Status.Active);
            var cpq = dbContext.CompanyOfferings.Where(cp => cp.UnlistedType == (byte)BizSrt.Model.Offering.UnlistedType.Listed);
            ...
            var allItems = (from p in pq
                            join cp in cpq on p.Id equals cp.Offering
            ...
            {
                var pm = dbContext.OfferingMedia
                    .Where(m => m.Offering == id && m.Type == (byte)BizSrt.Model.MediaType.Default_Image)
                    .Select(m => m.Metadata)
                    .FirstOrDefault();
