...
{
    internal class CategoriesCache : TreeCache<short, CachedCategory.NameKey, CachedCategory, byte, BizSrt.Api.Data.AppDbContext>
    {
        ...
        public BizSrt.Api.Model.Group.NodeRef<short>[] GetChildren(short parentCategory, short lookupCategory)
        {
            return GetChildren(parentCategory, lookupCategory, 0, category => new BizSrt.Api.Model.Group.NodeRef<short>
            {
                Id = category.Id,
                Name = category.Name,
        ...
        {
            base.Drop(key);
            BizSrt.Api.Data.Cache.LegacyCache.CategorySearch.Reset();
        }
    ...
    internal class CachedCategory : CachedNode<short>
    {
        ...
        internal BizSrt.Api.Model.Category Config
        {
            get
            {
                ...
                var q = from ca in categoryAttributes
                        join at in BizSrt.Api.Data.Cache.LegacyCache.Dictionary.Get<Model.Product.Attribute.Type>(BizSrt.Api.Model.DictionaryType.ProductAttributeType) on ca.Type equals at.ItemKey
                        select new BizSrt.Api.Model.Category.ProductAttribute { Type = at.ItemKey, Name = ca.Name, EditorType = at.EditorType, ValueType = at.ValueType, DefaultValue = ca.DefaultValue ?? at.DefaultValue, ValueOptions = ca.ValueOptions ?? at.ValueOptions, Requirement = (Model.Product.Attribute.Requirement)ca.Requirement };

                var productAttributes = q.ToArray();
                return new BizSrt.Api.Model.Category { Service = ServiceType, Product = ProductType, Transaction = TransactionType, Industry = Industry, ProductAttributes = productAttributes.Length > 0 ? productAttributes : null };
            }
        }
        #endregion
        ...
        public override CachedNode<short, byte> Parent
        {
            get
            {
                return ParentKey >= 0 ? BizSrt.Api.Data.Cache.LegacyCache.Categories[ParentKey] : null;
            }
        }
        ...
        {
            get { return _qualifyingParent > 0 ? BizSrt.Api.Data.Cache.LegacyCache.Categories[_qualifyingParent] : null; }
        }
        ...
        public override BizSrt.Api.Model.Group.Node<short> ParentNode(byte type)
        {
            if (ParentKey >= 0)
            {
                var parents = this.GetPath(null).Where(c => c.Id != _id).Select(c => c.Id);
                var parentCategory = BizSrt.Api.Data.Cache.LegacyCache.Categories[ParentKey];
                return new BizSrt.Api.Model.Group.Node<short> { Id = ParentKey, Name = parentCategory.Name, NodeType = parentCategory.NodeType(type), Parent = parentCategory.ParentNode(type) };
            }
            else
                return null;
        }
        ...
        public override NodeType NodeType(byte type)
        {
            ...
            else if (((type & (byte)BizSrt.Api.Model.Category.MemberType.Company) > 0 && _serviceType > 0) ||
                    ((type & (byte)BizSrt.Api.Model.Category.MemberType.Product) > 0 && _productType > 0))
                isClass = true;

            return isClass ? BizSrt.Api.Model.Group.NodeType.Class : BizSrt.Api.Model.Group.NodeType.Super;
        }

        internal Category ToModel(BizSrt.Api.Model.Group.DisplayType type)
        {
