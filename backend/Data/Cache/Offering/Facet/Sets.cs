...
namespace BizSrt.Api.Data.Cache.Offering.Facet
{
    public class SetsCache : TwoKeyExpirationCache<int, CachedSet.Key, CachedSet, AppDbContext>
    {
        public SetsCache()
            ...
            {
                var searchInput = data as BizSrt.Model.Offering.SearchInput;
                ...
                {
                    var pfs = new CompanyOfferingFacetSet();
                    pfs.Key = key2.Value; 
                    pfs.InclFacets = (byte)(searchInput.InclFacets != null ? searchInput.InclFacets.NoFilters : 0);
                    pfs.UseCount = 0;
                    pfs.LastUsed = DateTime.UtcNow;

                    dc.CompanyOfferingFacetSets.Add(pfs);

                    if (searchInput.InclFacets != null && searchInput.InclFacets.NoFilters > 0)
                    {
                        for (int i = 0; i < searchInput.InclFacets.NoFilters; i++)
                        {
                            dc.CompanyOfferingFacetSetDetails.Add(new CompanyOfferingFacetSetDetail { CompanyOfferingFacetSet = pfs, Value = searchInput.InclFacets.FilterValues[i], Exclude = false });
                        }
                    }
                    ...
                    if (searchInput.ExclFacets != null && searchInput.ExclFacets.NoFilters > 0)
                    {
                        ...
                        {
                            dc.CompanyOfferingFacetSetDetails.Add(new CompanyOfferingFacetSetDetail { CompanyOfferingFacetSet = pfs, Value = searchInput.ExclFacets.FilterValues[i], Exclude = true });
                        }
                    }
        ...
        protected override int Created(AppDbContext dc, BizSrt.Foundation.Cache.IKey<int> set, object data)
        {
            _ = System.Threading.Tasks.Task.Run(async () =>
            {
                try
                {
                    ...
                    await BizSrt.Api.Process.Company.IndexOfferingFacetSetAsync(localDc, set.Key, default);
                }
                catch (Exception ex)
                {
                ...
                if (set != null && set.Indexed)
                {
                    ...
                    if ((set.UseCount % 10) == 0 || set.LastUsed < DateTime.UtcNow.AddHours(-1))
                    {
                        ...
                        _ = System.Threading.Tasks.Task.Run(async () =>
                        {
                            ...
                            {
                                using var dc = BizSrt.Api.Data.Cache.LegacyCache.GetDbContext();
                                var facetSet = await Microsoft.EntityFrameworkCore.EntityFrameworkQueryableExtensions.SingleOrDefaultAsync(dc.CompanyOfferingFacetSets, las => las.Id == setId);
                                if (facetSet != null)
                                {
