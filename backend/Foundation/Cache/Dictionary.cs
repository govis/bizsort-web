...
namespace BizSrt.Api.Foundation.Cache
{
    public class Dictionary : ReadOneCache<DictionaryType, DictionaryItem[]> //Dictionary<DictionaryType>
    {
        ...
        protected static DictionaryItem[] fetchSecurityProfiles(DictionaryType type)
        {
            using (var dc = BizSrt.Api.Data.Cache.LegacyCache.GetDbContext())
            {
                ...
                var profiles1 = from sp in dc.SecurityProfiles.AsEnumerable()
                                ...
                                orderby sppg.Key.Id
                                select sppg;
                var profiles2 = new List<BizSrt.Api.Model.SecurityProfile>();
                foreach (var sp in profiles1)
                {
                    var model = new BizSrt.Api.Model.SecurityProfile { ItemKey = (BizSrt.Api.Model.SecurityProfile.Type)sp.Key.Id, Name = sp.Key.Name };
        ...
        private static void populateSecurityProfile(BizSrt.Api.Model.SecurityProfile model, IGrouping<BizSrt.Api.Data.Entities.SecurityProfile, BizSrt.Api.Data.Entities.SecurityProfilePriviledge> priviledges, IEnumerable<IGrouping<BizSrt.Api.Data.Entities.SecurityProfile, BizSrt.Api.Data.Entities.SecurityProfilePriviledge>> profiles)
        {
            ...
            {
                if (spp != null)
                    model[(BizSrt.Api.Model.SecurityPriviledge)spp.Priviledge] = !spp.Restricted;
            }
        }
