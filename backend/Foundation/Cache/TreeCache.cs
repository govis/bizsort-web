...
namespace BizSrt.Api.Foundation.Cache
{
    public abstract class CachedNode<TKey, TMemberType> : PartCache, IKey<TKey>, IMemberType<TMemberType> where TKey : struct, IComparable
    {
        ...
        public abstract BizSrt.Api.Model.Group.NodeType NodeType(TMemberType type);
        ...
        public abstract BizSrt.Api.Model.Group.Node<TKey> ParentNode(TMemberType type);

        public BizSrt.Api.Model.Group.IdName<TKey> IdName
        {
            get
            {
                return new BizSrt.Api.Model.Group.IdName<TKey> { Id = _id, Name = Name, NodeType = NodeType(default(TMemberType)) };
            }
        }

        public BizSrt.Api.Model.Group.IdName<TKey>[] GetPath(BizSrt.Api.Model.IEntityRef<TKey> root)
        {
            return GetPath(root, (cachedClass) => new BizSrt.Api.Model.Group.IdName<TKey> 
            { 
                Id = cachedClass.Id, 
        ...
        public TMember[] GetPath<TMember>(BizSrt.Api.Model.IEntityRef<TKey> root, Func<CachedNode<TKey, TMemberType>, TMember> populate)
            where TMember : BizSrt.Api.Model.IEntityRef<TKey>
        {
        ...
        public static BizSrt.Api.Model.Group.Node<TKey> PopulateWithChildren<TMember>(TKey classId, BizSrt.Api.Model.Group.SubType type, TMemberType memberTypes, ITreeCache<TKey, TMember, TMemberType> cache)
            where TMember : CachedNode<TKey, TMemberType>, IKey<TKey>
        {
            var zero = default(TKey);
            var cachedClass = cache[classId];

            if ((type & BizSrt.Api.Model.Group.SubType.Siblings) > 0 && classId.CompareTo(zero) > 0)
            {
                classId = cachedClass.ParentKey;
                cachedClass = cache[classId];
            }

            //var model = new BizSrt.Api.Model.Group.Node<TKey> { Id = classId, Name = cachedClass.Name, NodeType = cachedClass.NodeType(memberTypes), Parent = (classId.CompareTo(zero) > 0 ? cachedClass.ParentModel(memberTypes) : null) };
            var model = cachedClass.ToNode(memberTypes);
            if (classId.CompareTo(zero) > 0)
                model.Parent = cachedClass.ParentNode(memberTypes);

            if ((type & BizSrt.Api.Model.Group.SubType.Children) > 0 && cache.HasChildren(classId, memberTypes))
            {
                BizSrt.Api.Model.Group.Node<TKey> childClass;
                TMember[] children;
                var classes = new List<BizSrt.Api.Model.Group.Node<TKey>>();
                foreach (var cachedChild in cache[classId, memberTypes])
                {
                    //childClass = new BizSrt.Api.Model.Group.Node<TKey> { Id = cachedChild.Id, Name = cachedChild.Name, NodeType = cachedChild.NodeType(memberTypes) };
                    childClass = cachedChild.ToNode(memberTypes);
                    childClass.Parent = model;
                    
                    if ((type & BizSrt.Api.Model.Group.SubType.GrandChildren) > 0 && cache.HasChildren(cachedChild.Id, memberTypes))
                    {
                        children = cache[cachedChild.Id, memberTypes];
                        if (children != null && children.Length > 0)
                        {
                            //childClass.Children = children.Select(cc => new BizSrt.Api.Model.Group.Node<TKey> { Id = cc.Id, Name = cc.Name, NodeType = cc.NodeType(memberTypes), HasChildren = cache.HasChildren(cc.Id, memberTypes), Parent = childClass }).ToArray();
        ...
        public virtual BizSrt.Api.Model.Group.Node<TKey> ToNode(TMemberType type)
        {
            return new BizSrt.Api.Model.Group.Node<TKey> { Id = Id, Name = Name, NodeType = NodeType(type) };
        }

        public T ToModel<T>(BizSrt.Api.Model.Group.DisplayType type) where T : BizSrt.Api.Model.IEntityRef<TKey>, new()
        {
            T model = new T() { Id = Id, Name = Name };
            switch (type)
            {
                //case global::BizSrt.Api.Model.Group.DisplayType.Name:
                //    model.Name = Name;
                //    break;
                case BizSrt.Api.Model.Group.DisplayType.Text:
                    model.Name = DisplayText;
                    break;
                case BizSrt.Api.Model.Group.DisplayType.Path:
    ...
    public abstract class CachedNode<T> : CachedNode<T, byte> where T : struct, IComparable
    {
        ...
        public TMember[] GetPath<TMember>(BizSrt.Api.Model.IEntityRef<T> root, Func<CachedNode<T>, TMember> populate)
            where TMember : BizSrt.Api.Model.IEntityRef<T>
        {
        ...
        public TMember EntityRef<TMember>(BizSrt.Api.Model.Group.DisplayType type, Action<CachedNode<T>, TMember> populate)
            where TMember : BizSrt.Api.Model.IEntityRef<T>, new()
        {
        ...
        where TModel : DbContext
    {
        ...
        public T[] GetChildren<T>(TKey parentKey, TKey lookupKey, TMemberType memberTypes, Func<TMember, T> populate, ITreeCache<TKey, TMember, TMemberType> cache)
            where T : BizSrt.Api.Model.Group.NodeRef<TKey>
        {
