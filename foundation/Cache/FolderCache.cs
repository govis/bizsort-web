...
namespace BizSrt.Foundation.Cache
{
        ...
        where TFolderKey : IComparable
    {
        ...
        public T[] GetChildren<T>(TFolderKey parentKey, TFolderKey lookupKey, Func<TFolder, T> populate)
            where T : /*BizSrt.Model.Group.NodeRef<TFolderKey>*/IEntityId<TFolderKey>, BizSrt.Model.Group.IChildren<T>, new()
        {
        ...
        public T[] GetChildren<T, TChild>(TFolderKey parentKey, TFolderKey lookupKey, Func<TFolder, T> populate, Func<TFolder, TChild> populateChild = null) 
            where T : /*BizSrt.Model.Group.NodeRef<TFolderKey>*/IEntityId<TFolderKey>, BizSrt.Model.Group.IChildren<TChild>, new()
            ...
            else if (!_isFlat)
            {
                ...
                if (lookupKey.CompareTo(default(TFolderKey)) > 0)
                {
                    ...
                    if (cachedFolder == null)
                    {
                        ...
                        var folderPath = GetPath<BizSrt.Model.Group.NodeRef<TFolderKey>>(lookupKey);
        ...
        IEnumerable<TFolder> populateChildren<T, TChild>(T lookupFolder, Func<TFolder, TChild> populate)
            where T : /*BizSrt.Model.Group.NodeRef<TFolderKey>*/IEntityId<TFolderKey>, BizSrt.Model.Group.IChildren<TChild>
            where TChild : IEntityId<TFolderKey>
        {
        ...
        /*public T[] GetChildren<T>(TFolderKey parentKey, TFolderKey lookupKey, Func<TFolder, T> populate)
            where T : BizSrt.Model.Group.NodeRef<TFolderKey>, new()
        {
            if (_isFlat && parentKey.CompareTo(default(TFolderKey)) == 0)
                return _cache.Select(populate).ToArray();
            else if (!_isFlat)
        ...
        T[] populateChildren<T>(T lookupFolder, Func<TFolder, T> populate)
            where T : BizSrt.Model.Group.NodeRef<TFolderKey>
        {
        ...
        protected T GetDisplayPath<T>(TFolderKey key) where T : BizSrt.Model.IEntityRef<TFolderKey>, new()
        {
            TFolder node;
        ...
        protected T[] GetPath<T>(TFolderKey key) where T : BizSrt.Model.Group.NodeRef<TFolderKey>, new()
        {
            var path = new List<T>();
            getPath(path, key);
            return path.ToArray();
        ...
        void getPath<T>(List<T> path, TFolderKey key) where T : BizSrt.Model.Group.NodeRef<TFolderKey>, new()
        {
