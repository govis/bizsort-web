...
namespace BizSrt.Data.Company;

public interface ICompanyProductService
{
    Task<SearchOutput<SearchItem>> SearchAsync(BizSrt.Model.Product.SearchInput queryInput);
...
public class CompanyProductService : ICompanyProductService
{
    ...
    public Task<SliceOutput<SearchItem>> GetFeaturedAsync(DirectorySliceInput<long> sliceInput)
    {
        ...
        long product;
        var cached = BizSrt.Data.Cache.LegacyCache.FeaturedProducts[new Tuple<short, int>(sliceInput.Category, sliceInput.Location), sliceInput.Index == 0 && sliceInput.Length > 1];
    ...
    {
        var products = new List<long>();
        var cached = BizSrt.Data.Cache.LegacyCache.CompanyProfiles[company]?.Products ?? Array.Empty<long>();
        var index = sliceInput.Index;
        while (products.Count < sliceInput.Length && index < cached.Length)
        {
    ...
    public Task<Preview[]> ToPreviewAsync(SearchItem[] products, Dictionary<string, object> options)
    {
        if (products != null && products.Length > 0)
        {
            ...
            var cachedProducts = BizSrt.Data.Cache.LegacyCache.CompanyProducts[productIds, false];
            
            if (options?.ContainsKey("company") == true)
            {
                var companies = BizSrt.Data.Cache.LegacyCache.CompanyProfiles[cachedProducts.Select(p => p.CompanyId).Distinct().ToArray()]
    ...
    public async Task<SearchOutput<SearchItem>> SearchAsync(BizSrt.Model.Product.SearchInput queryInput)
    {
    ...
    private async Task<SearchOutput<SearchItem>> ExecuteProductSearchSpAsync(BizSrt.Model.Product.SearchInput queryInput)
    {
