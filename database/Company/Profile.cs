...
using BizSrt.Data.Cache.Company;
...
public class CompanyService(AppDbContext dbContext) : ICompanyService
{
    ...
    public Task<SliceOutput<SearchItem>> GetFeaturedAsync(DirectorySliceInput<int> sliceInput)
    {
        ...
        int company;
        var cached = BizSrt.Data.Cache.LegacyCache.FeaturedCompanies[new Tuple<short, int>(sliceInput.Category, sliceInput.Location), sliceInput.Index == 0 && sliceInput.Length > 1];
    ...
    public async Task<IEnumerable<BizSrt.Model.Company.Preview>> ToPreviewAsync(SearchItem[] companies)
    {
        ...
        var cachedProfiles = BizSrt.Data.Cache.LegacyCache.CompanyProfiles[ids];
