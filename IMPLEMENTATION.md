...
The frontend uses **Astro** as the host framework but utilizes **Lit** for the core UI logic to facilitate a smooth port from the legacy Polymer components.

### UI Stack:
- **Astro 4**: Islands architecture, file-based routing, and static/SSR.
...
### EF Core 10 Optimizations & Data Modeling
- **LINQ Query Optimization**: Legacy EF queries often relied on `.DefaultIfEmpty()` (Left Outer Join) patterns to count matches (e.g., in `Entity.Product.Get` and `ProfileQueryExtensions`). These generated sub-optimal execution plans or `DbIsNullExpression` errors in LINQ to Entities. The modern data layer optimizes these by using `.Where(cf => ...).Take(1).DefaultIfEmpty()` for exclusory `NOT EXISTS` filters and `.GroupBy` with `Inner Joins` for required intersection filters, dramatically improving SQL server execution plans.
- **Join Table Primary Keys**: Modern many-to-many join tables (like `FacetSetCompanyProduct`) without an explicit surrogate key are explicitly mapped with composite `[PrimaryKey(nameof(FacetSet), nameof(Product))]` attributes to satisfy EF Core 10 requirements and optimize clustering behavior. Note that these pure intersection tables **do not** require artificial surrogate `Id` columns because they carry no additional metadata payload, making composite uniqueness both sufficient and optimal.
...
## 7. Search Input Flow Architecture

### Search Category Input Flow
