...
ALTER PROCEDURE [dbo].[CompanySearch] 
...
--http://www.sommarskog.se/dyn-search.html
	...
	(	
		...
		LEFT JOIN (SELECT CPt.Company, MAX(COALESCE(Pt.[Rank], 0)) AS ProductRank 
			...
			FROM CompanyOfferings AS CPt WITH(NOLOCK)
                                                                                                                                                                                                                  
			LEFT JOIN Offerings AS P WITH(NOLOCK) ON CPt.Offering = P.Id
                                                                                                                                                                                                
			LEFT JOIN ProductTextSearch2(@Query) AS Pt ON @TextQuery = 1 AND CPt.Offering = Pt.[KEY]
                                                                                                                                                                   
			WHERE (P.[Type] = 0/*Multiproduct*/ OR (CPt.[UnlistedType] = 0/*Listed*/ AND P.[Status] = 1/*Active*/)) AND
                                                                                                                                               
			(@TextQuery = 0 OR Pt.[KEY] IS NOT NULL) AND
                                                                                                                                                                                                              
			(@Category = 0 /*Bug! OR P.[Type] = 0/*Multiproduct*/*/ OR CPt.Category = @Category OR
